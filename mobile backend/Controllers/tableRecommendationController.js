  ...
  try {
    ...
    const interaction = new TableInteraction({
      ...
      deviceType: req.headers["user-agent"]?.includes("Mobile")
        ? "mobile"
        : "mobile", // Default to mobile for mobile app
    ...
    const enrichedRecommendations = await Promise.all(
      mlRecommendations.map(async (rec, index) => {
        ...
        if (!table) {
          ...
          const tableNumber = rec.tableId
            ? parseInt(rec.tableId.replace(/\D/g, ""))
            : null;
        ...
        return {
          ...
          explanation:
            rec.explanation ||
            `This table matches your preferences for ${validatedOccasion}`,
          contextFactors: {
            occasion: validatedOccasion,
